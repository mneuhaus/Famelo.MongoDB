...
class PersistedUsernamePasswordProvider extends \TYPO3\Flow\Security\Authentication\Provider\AbstractProvider {
	...
	/**
	 * @Flow\Inject
	 * @var \Famelo\MongoDB\Persistence\DocumentManager
	 */
	...
	public function authenticate(TokenInterface $authenticationToken) {
		...
		if (is_array($credentials) && isset($credentials['username'])) {
			...
			$this->securityContext->withoutAuthorizationChecks(function() use ($credentials, $providerName, $documentManager, &$account) {
				$account = $this->documentManager->getRepository('\Famelo\MongoDB\Security\Account')->findOneBy(array('accountIdentifier' => $credentials['username']));
			});
		}
